"""PINN-E2CO model — same architecture as MSE2C with cleaner interface."""
...
class PINNE2CO(nn.Module):
    """PINN-E2CO model — identical architecture to MSE2C, cleaner interface."""
    ...
    def forward(self, inputs):
        ...
        Returns:
            dict with keys: x0_rec, X_next_pred, X_next, Z_next_pred,
                           Z_next, Y_next_pred, Y, z0
        """
        X, U, Y, dt = inputs
    ...
    def load_from_legacy(self, encoder_file, decoder_file, transition_file,
                         map_location='cpu'):
        """Load existing MSE2C .h5 weights."""
        self.encoder.load_state_dict(
            torch.load(encoder_file, map_location=map_location, weights_only=False))
        self.decoder.load_state_dict(
            torch.load(decoder_file, map_location=map_location, weights_only=False))
        self.transition.load_state_dict(
            torch.load(transition_file, map_location=map_location, weights_only=False))
