"""Data loading — reimplements data_preprocessing.py with permeability support."""
...
class PINNDataLoader:
    """Loads and splits data for PINN-E2CO training."""

    def __init__(self, cfg):
        self.cfg = cfg
        self.device = None  # set in load_all()

    def load_all(self, device):
        """Load all data, return train/eval tensors + permeability field.

        Returns:
            train_data: dict with STATE, BHP, Yobs, dt
            eval_data:  dict with STATE, BHP, Yobs, dt
            perm:       (1, Nx, Ny) tensor, log-scale permeability
        """
        self.device = device
        cfg = self.cfg
    ...
    def load_test_data(self, device):
        ...
        Returns:
            test_data: dict with sat, pres, bhp_seq, yobs_seq, test_case indices
            perm: (1, Nx, Ny) permeability tensor
        """
        self.device = device
        cfg = self.cfg
        ...
        # Test case indices (same pattern as SC notebook cell-38)
        ...
        # Reshape controls for sequential eval
        num_tstep = 20
    ...
    def _load_permeability(self):
        """Load permeability: slice [:64, 120:184], log-scale."""
        cfg = self.cfg
    ...
    def _prepare_data(self):
        """Reimplements prepare_data() from data_preprocessing.py."""
        cfg = self.cfg
        Ksteps = cfg.nsteps
    ...
    def _train_split_data(self, Mole_slt, SAT_slt, PRES_slt, BHP_slt,
                          ...
                          num_prod, num_inj):
        """Reimplements train_split_data() from data_preprocessing.py."""
        cfg = self.cfg
        device = self.device
        ...
        for i_step in range(len(SAT_slt)):
            ...
            if cfg.n_channels == 3:
                ...
                for k in range(split_ratio):
                    ind0 = k * num_run_per_case
                    sat_t_train[ind0:ind0 + num_run_per_case, ...] = \
                        SAT_slt[i_step][k * 100:k * 100 + num_run_per_case, ...]
                    ind1 = k * num_run_eval
                    sat_t_eval[ind1:ind1 + num_run_eval, ...] = \
                        SAT_slt[i_step][k * 100 + num_run_per_case:k * 100 + 100, ...]
                SAT_t_train = sat_t_train.reshape(
                    (num_run_per_case * split_ratio * num_t_slt, 1, Nx, Ny))
                SAT_t_eval = sat_t_eval.reshape(
                    (num_run_eval * split_ratio * num_t_slt, 1, Nx, Ny))
                STATE_t_train = torch.tensor(
                    np.concatenate((Mole_t_train, SAT_t_train, PRES_t_train), axis=1),
                    dtype=torch.float32).to(device)
                STATE_t_eval = torch.tensor(
                    np.concatenate((Mole_t_eval, SAT_t_eval, PRES_t_eval), axis=1),
                    dtype=torch.float32).to(device)
            else:
                STATE_t_train = torch.tensor(
