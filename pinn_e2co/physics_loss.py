"""Physics-informed losses + adaptive weighting for PINN-E2CO."""
...
class AdaptiveLossWeights(nn.Module):
    ...
    def forward(self, losses_dict):
        """Combine losses with adaptive weighting. Returns (total_loss, None).
        Weight info is deferred to avoid .item() calls during training."""
...
class PINNLoss(nn.Module):
    """Combined data-driven + physics-informed loss for PINN-E2CO.
    ...
    def __init__(self, cfg, perm_log, device):
        ...
        # Precompute inverse-sum transmissibility for Darcy flux (matches MSloss.py pattern)
    ...
    def forward(self, pred):
        """Compute all 8 loss terms. Returns (total_loss, losses_tuple).

        losses_tuple is a tuple of 8 tensors (no .item() calls during training).
        Call get_loss_dict() separately at logging time.
        """
        x0 = pred['x0']
        x0_rec = pred['x0_rec']
        z0 = pred['z0']
    ...
    def losses_to_dict(self, losses_stack):
        """Convert losses tuple to dict with .item() calls — only for logging."""
    ...
    def _darcy_flux_loss(self, x_true, x_pred):
        ...
        # Use precomputed darcy_tran buffers (unsqueeze for batch broadcast)
        tx = self.darcy_tran_x  # (1, 63, 64)
        ty = self.darcy_tran_y  # (1, 64, 63)
...
# ---- Standalone loss functions (no self, can be compiled) ----
