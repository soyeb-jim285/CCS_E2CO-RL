"""PINN-E2CO configuration dataclass with CLI argument parsing."""
...
@dataclass
class PINNConfig:
    # --- Data paths (relative to pinn_e2co/ directory) ---
    data_dir: str = "../data/"
    ...
    cond: str = "SC"  # standard condition for rates

    # --- Model architecture ---
    latent_dim: int = 20
    u_dim: int = 9  # 5 producers + 4 injectors
    num_prod: int = 5
    num_inj: int = 4
    n_channels: int = 2  # mole fraction + pressure
    Nx: int = 64
    Ny: int = 64
    nsteps: int = 2  # K-step prediction horizon
    input_shape: Tuple[int, int, int] = (2, 64, 64)

    # --- Training ---
    epochs: int = 200
    batch_size: int = 32  # original MSE2C used 4, but GPU can handle much more
    learning_rate: float = 2e-4
    seed: int = 1010

    # --- Performance ---
    use_amp: bool = True            # mixed precision (fp16) — ~2x speedup
    use_compile: bool = True        # torch.compile — 10-30% speedup on PyTorch 2.0+
    use_tensorboard: bool = True
    gradient_accumulation_steps: int = 1  # effective_batch = batch_size * this
    eval_every: int = 5             # eval every N epochs (not every epoch)
    eval_batch_size: int = 256      # batch eval instead of full-set forward

    # --- Checkpoint / resume ---
    checkpoint_every: int = 20  # save checkpoint every N epochs
    ...
    adaptive_lr: float = 1e-3  # learning rate for adaptive weight params

    # --- Physics PDE params ---
    porosity: float = 0.2  # average porosity
    total_compressibility: float = 1e-5  # ct in 1/psi
    dt_physical: float = 20.0  # days per timestep

    # --- Evaluation ---
    test_cases_per_batch: int = 25  # 25 per 100-case batch → 100 total
    ind_case: List[int] = field(
        default_factory=lambda: [10, 25, 77, 97]
    )
    eval_case: int = 77  # well output plot case

    @classmethod
    def from_args(cls, args=None):
        """Create config from CLI arguments, overriding defaults."""
        parser = argparse.ArgumentParser(description="PINN-E2CO Training")
