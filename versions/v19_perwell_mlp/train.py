"""V19 Per-Well MLP training script.

Load V0 pretrained weights → fine-tune with per-well MLP observation heads.
Encoder/decoder from V0, transition dynamics (A/B) from V0, well heads are fresh.
"""
...
from versions.v19_perwell_mlp.model import PerWellMLPE2CO
from versions.v19_perwell_mlp.loss import PerWellMLPLoss
...
def add_extra_args(parser):
    ...
    parser.add_argument("--lambda_yobs", type=float, default=5.0)
...
cfg.version_name = "v19_perwell_mlp"

print(f"=== V19 Per-Well MLP Observation Heads ===")
...
print(f"lambda_yobs: {cfg.lambda_yobs}")
...
# Partial checkpoint loading: encoder, decoder, transition.{trans_encoder, At, Bt} match V0
# transition.{Ct, Dt} skipped (no longer exist), well_heads/inj_head are fresh
...
loss_fn = PerWellMLPLoss(cfg, perm, device).to(device)
