"""V17 Fine-Tune + Structured ODE training script.

Load V0 pretrained weights → fine-tune with ODE transition + differential LR.
Encoder/decoder from V0, transition (ODE) is fresh.
"""
...
from versions.v17_finetune_ode.model import FinetuneODEE2CO
from versions.v17_finetune_ode.loss import FinetuneODELoss
...
# Partial checkpoint loading: encoder + decoder match V0, transition (ODE) is fresh
...
if pretrained_loaded:
    print(f"Fine-tuning with differential LR: "
          ...
          f"transition(ODE)={cfg.learning_rate:.2e}")
