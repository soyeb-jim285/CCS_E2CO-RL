"""V5 Coordinate PINN E2CO — dual decoder with CNN + coordinate-based MLP."""
...
class CoordinatePINNDecoder(nn.Module):
    """MLP decoder: (z, x, y) -> (pressure, saturation) at coordinate points.
    Used for autograd-based physics losses at collocation points."""
...
class CoordinatePINNE2CO(nn.Module):
    """V5 Coordinate PINN E2CO — dual decoder (CNN + coordinate MLP)."""

    def __init__(self, latent_dim, u_dim, num_prod, num_inj, input_shape,
                 nsteps=2, sigma=0.0, n_collocation_points=256):
        super().__init__()
        self.latent_dim = latent_dim
        self.nsteps = nsteps
        ...
        self.pinn_decoder = CoordinatePINNDecoder(latent_dim, hidden_dim=128,
                                                   out_channels=input_shape[0])
        self.transition = LinearTransitionModel(latent_dim, u_dim,
                                                 num_prod, num_inj)

        # Register full 64x64 grid coordinates normalized to [-1, 1]
    ...
    def _sample_collocation_coords(self, batch_size, device):
        """Sample random collocation points from the full grid."""
        n_total = self.grid_coords.shape[0]
        indices = torch.randint(0, n_total, (self.n_collocation_points,), device=device)
        coords = self.grid_coords[indices]  # (n_collocation, 2)
        # Expand for batch and enable grad
    ...
    def forward(self, inputs):
        ...
        Returns:
            dict with standard keys plus 'pinn_pred', 'collocation_coords'
        """
        X, U, Y, dt = inputs
        ...
        for i_step in range(len(Z_next_pred)):
            ...
            # PINN decoder at collocation points
        ...
        # Also run PINN decoder for t=0 reconstruction
    ...
    def predict(self, xt, ut, yt, dt):
        """Single-step prediction for sequential eval (CNN decoder only)."""
        self.eval()
        with torch.no_grad():
            zt = self.encoder(xt)
