"""V7 DeepONet E2CO training script — DeepONet decoder with autograd/FD physics losses."""
...
from versions.v7_deeponet.model import DeepONetE2CO
from versions.v7_deeponet.loss import DeepONetPhysicsLoss


def add_extra_args(parser):
    parser.add_argument("--n_basis", type=int, default=64)
    parser.add_argument("--trunk_dim", type=int, default=128)
    return {'n_basis': 64, 'trunk_dim': 128}


# Parse CLI args
cfg = BaseConfig.from_args(extra_args_fn=add_extra_args)
cfg.version_name = "v7_deeponet"

print(f"=== V7 DeepONet E2CO ===")
...
print(f"DeepONet: n_basis={cfg.n_basis}, trunk_dim={cfg.trunk_dim}")
...
loss_fn = DeepONetPhysicsLoss(cfg, perm, device).to(device)
