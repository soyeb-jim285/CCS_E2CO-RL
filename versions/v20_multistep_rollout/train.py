"""V20 Multi-Step Rollout training script.

Load V0 pretrained weights → fine-tune with nsteps=6 rollout + differential LR.
Architecture identical to V0, but trained with 6-step rollout windows.
"""
...
from versions.v20_multistep_rollout.model import PhysicsConstrainedE2CO
from versions.v20_multistep_rollout.loss import MultistepLoss
...
cfg.version_name = "v20_multistep_rollout"
cfg.nsteps = 6  # Override BEFORE data loading — creates 6-step windows
cfg.batch_size = 16  # More memory needed for 6-step rollout

print(f"=== V20 Multi-Step Rollout (nsteps={cfg.nsteps}) ===")
...
# Full V0 checkpoint load — architecture identical, strict=True works
...
if os.path.exists(cfg.pretrained_path):
    ...
    print("  Pretrained weights loaded successfully (full match).")
else:
    print(f"WARNING: Pretrained checkpoint not found at {cfg.pretrained_path}")
    print("  Training from scratch.")

loss_fn = MultistepLoss(cfg, perm, device).to(device)
