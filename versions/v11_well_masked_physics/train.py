"""V11 Well-Masked Physics E2CO training script."""
...
from versions.v11_well_masked_physics.model import PhysicsConstrainedE2CO
from versions.v11_well_masked_physics.loss import WellMaskedPhysicsLoss


def add_extra_args(parser):
    parser.add_argument("--well_mask_radius", type=int, default=3)
    return {"well_mask_radius": 3}


cfg = BaseConfig.from_args(extra_args_fn=add_extra_args)
cfg.version_name = "v11_well_masked_physics"

print(f"=== V11 Well-Masked Physics E2CO ===")
...
print(f"Well mask radius: {cfg.well_mask_radius}")
...
loss_fn = WellMaskedPhysicsLoss(
    cfg, perm, device,
    well_mask_radius=cfg.well_mask_radius,
).to(device)
