"""Evaluation and visualization — identical for all versions.

Generates SC-matching plots + PINN-specific metrics.
Saves summary_metrics.json for cross-version comparison.
"""
...
from .utils import compute_rmse, compute_mae, compute_r2, denormalize_rates


class BaseEvaluator:
    """Generates all plots and saves summary metrics."""
    ...
    def run_sequential_eval(self, test_data, perm):
        """Run sequential prediction on 100 test cases."""
        cfg = self.cfg
        model = self.model
        model.eval()
    ...
    def save_summary_metrics(self, test_data, results, train_time=None):
        """Compute and save summary metrics to JSON for comparison."""
        cfg = self.cfg
        ...
        # Saturation metrics (normalized)
        sat_pred_np = state_pred[:, :, 0, :, :].cpu().numpy()
        sat_true_np = sat_seq_true[:, :num_tstep, ...].numpy()
        sat_rmse = float(np.sqrt(np.mean((sat_pred_np - sat_true_np) ** 2)))
        sat_mae = float(np.mean(np.abs(sat_pred_np - sat_true_np)))

        # Pressure metrics (psia)
        pres_pred_np = state_pred[:, :, 1, :, :].cpu().numpy() * p_diff + cfg.p_min
        pres_true_np = pres_seq_true[:, :num_tstep, ...].numpy() * p_diff + cfg.p_min
        pres_rmse = float(np.sqrt(np.mean((pres_pred_np - pres_true_np) ** 2)))
        pres_mae = float(np.mean(np.abs(pres_pred_np - pres_true_np)))

        # Per-timestep RMSE
        sat_rmse_per_t = []
        pres_rmse_per_t = []
        for t in range(num_tstep):
            sat_rmse_per_t.append(float(np.sqrt(np.mean(
                (sat_pred_np[:, t] - sat_true_np[:, t]) ** 2))))
            pres_rmse_per_t.append(float(np.sqrt(np.mean(
                (pres_pred_np[:, t] - pres_true_np[:, t]) ** 2))))

        # Well R2 values
        yobs_pred_d = yobs_pred.cpu()
        yobs_true_d = test_data['yobs_t_seq'].cpu()
        num_prod = cfg.num_prod
        Q_max_w = cfg.Q_max_w
        Q_max_g = cfg.Q_max_g

        yp = yobs_pred_d.clone()
        yt = yobs_true_d.clone()
        yp[:, :, :num_prod] *= Q_max_w
        yt[:, :, :num_prod] *= Q_max_w
        yp[:, :, num_prod:2*num_prod] *= Q_max_g
        yt[:, :, num_prod:2*num_prod] *= Q_max_g
        yp[:, :, 2*num_prod:] = yp[:, :, 2*num_prod:] * p_diff + cfg.p_min
        yt[:, :, 2*num_prod:] = yt[:, :, 2*num_prod:] * p_diff + cfg.p_min

        # Cumulative production R2
        cum_pred = torch.sum(yp[:, :, :num_prod * 2], dim=1).numpy()
        cum_true = torch.sum(yt[:, :, :num_prod * 2], dim=1).numpy()
        well_r2 = {}
        for i in range(cum_pred.shape[-1]):
            name = f"water_P{i+1}" if i < num_prod else f"gas_P{i-num_prod+1}"
            well_r2[name] = float(compute_r2(cum_pred[:, i], cum_true[:, i]))

        # Final training loss from CSV
        final_loss = None
        ...
        if os.path.exists(csv_path):
            ...
            if rows:
                final_loss = float(rows[-1].get('train_loss', 0))

        metrics = {
            'version': cfg.version_name,
            'sat_rmse': sat_rmse,
            'sat_mae': sat_mae,
            'pres_rmse_psia': pres_rmse,
            'pres_mae_psia': pres_mae,
            'sat_rmse_per_timestep': sat_rmse_per_t,
            'pres_rmse_per_timestep': pres_rmse_per_t,
            'well_r2': well_r2,
            'avg_well_r2': float(np.mean(list(well_r2.values()))),
        ...
        out_path = os.path.join(cfg.log_dir, "summary_metrics.json")
        with open(out_path, 'w') as f:
            json.dump(metrics, f, indent=2)
        print(f"Summary metrics saved to {out_path}")
        return metrics
    ...
    def generate_all_plots(self, test_data, perm, results):
        ...
        if self.loss_fn is not None and hasattr(self.loss_fn, 'compute_physics_residuals'):
    ...
    def _plot_saturation_maps(self, state_pred, state_seq_true, test_data, plot_dir):
        ...
        sat_pred_plot = state_pred[:, :, 0, :, :].cpu()
        divide = 2
        for k_idx, k in enumerate(ind_case):
            ...
            for i_tstep in range(num_tstep // divide):
                ...
                error = torch.abs(state_seq_true[k, 0, t_idx, ...] - sat_pred_plot[k, t_idx, :, :].detach())
    ...
    def _plot_r2_crossplots(self, yobs_pred, yobs_seq_true, test_data, plot_dir):
        ...
        for i_well in range(cum_pred.shape[-1]):
            ...
            if i_well < num_prod:
                r2 = compute_r2(cum_pred[:, i_well] / 1000, cum_true[:, i_well] / 1000)
                ...
                r2 = compute_r2(cum_pred[:, i_well] / 1e6, cum_true[:, i_well] / 1e6)
            ...
            for r in rows:
                ...
                if v != '' and v is not None:
                    try:
                        ...
                        pass
            if vals:
                ax.plot(eps, vals, linewidth=1.5, label=key.replace('train_', ''))

        ax.set_xlabel('Epoch'); ax.set_ylabel('Loss')
        ax.set_title('Sub-Losses', fontweight='bold')
    ...
    def _plot_per_timestep_errors(self, state_pred, state_seq_true, test_data, plot_dir):
        ...
        axes[0].bar(x, sat_rmse, color='steelblue')
        axes[0].set_title('Saturation RMSE per Timestep')
        axes[0].set_xlabel('Timestep'); axes[0].set_ylabel('RMSE')

        axes[1].bar(x, pres_rmse, color='darkorange')
        axes[1].set_title('Pressure RMSE per Timestep (psia)')
    ...
    def _plot_error_histograms(self, state_pred, state_seq_true, test_data, plot_dir):
        ...
        axes[1].set_xlabel('Error (psia)'); axes[1].set_ylabel('Density')
