"""V22 Full Combo training script.

Combines all winning ingredients: ODE dynamics + FNO decoder + well features + multi-step.
Load V0 pretrained weights → fine-tune with differential LR.
Only encoder weights match V0. Decoder (FNO), transition (ODE+MLP) all fresh.
"""
...
from versions.v22_full_combo.model import FullComboE2CO
from versions.v22_full_combo.loss import FullComboLoss
...
def add_extra_args(parser):
    ...
    parser.add_argument("--lambda_eigen_reg", type=float, default=0.001)
    parser.add_argument("--pretrained_path", type=str,
                        default="outputs_v0_baseline/checkpoints/best_model.pt")
    parser.add_argument("--encoder_lr_scale", type=float, default=0.05)
    return {"ode_method": "euler", "fno_modes": 12, "fno_width": 32,
...
cfg.version_name = "v22_full_combo"
cfg.nsteps = 6
cfg.batch_size = 8
cfg.use_amp = False      # torchdiffeq + cfloat incompatible with fp16
cfg.use_compile = False  # torchdiffeq + cfloat incompatible with torch.compile

print(f"=== V22 Full Combo (ODE + FNO + Well Features + Multi-Step) ===")
...
print(f"ODE method: {cfg.ode_method}, FNO: modes={cfg.fno_modes}, width={cfg.fno_width}")
...
# Partial checkpoint loading: only encoder weights match V0
...
loss_fn = FullComboLoss(
    cfg, perm, device,
    model=model,
    lambda_eigen_reg=cfg.lambda_eigen_reg,
).to(device)
...
class FinetuneTrainer(BaseTrainer):
    def __init__(self, model, loss_fn, cfg, device, encoder_lr_scale=0.05):
        self.model = model
        ...
        param_groups = [
            ...
            {'params': transition_params, 'lr': cfg.learning_rate},
        ]
...
if pretrained_loaded:
    print(f"Fine-tuning with differential LR: "
          ...
          f"FNO decoder={cfg.learning_rate:.2e}, "
          f"ODE transition={cfg.learning_rate:.2e}")
