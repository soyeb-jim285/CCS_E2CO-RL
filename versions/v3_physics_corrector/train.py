"""V3: Physics Corrector E2CO — Training entry point.

Run as: python -m versions.v3_physics_corrector.train [--epochs N] [--batch_size N] ...

Extra CLI args:
  --n_corrector_iterations  Number of Jacobi correction iterations (default: 2)
  --corrector_alpha         Correction step size (default: 0.1)
"""
...
from versions.v3_physics_corrector.model import PhysicsCorrectorE2CO
from versions.v3_physics_corrector.loss import PhysicsLoss


def add_v3_args(parser):
    parser.add_argument("--n_corrector_iterations", type=int, default=2)
    parser.add_argument("--corrector_alpha", type=float, default=0.1)
    return {"n_corrector_iterations": 2, "corrector_alpha": 0.1}


# --- Config ---
cfg = BaseConfig.from_args(extra_args_fn=add_v3_args)
cfg.version_name = "v3_physics_corrector"
cfg.batch_size = cfg.batch_size if cfg.batch_size != BaseConfig.batch_size else 32

print(f"=== V3: Physics Corrector E2CO ===")
...
print(f"Corrector: n_iterations={cfg.n_corrector_iterations}, "
      f"alpha={cfg.corrector_alpha}")
...
model = PhysicsCorrectorE2CO(
    ...
    perm_log=perm,
    device=device,
    n_corrector_iterations=cfg.n_corrector_iterations,
    corrector_alpha=cfg.corrector_alpha,
    porosity=cfg.porosity,
    total_compressibility=cfg.total_compressibility,
    dt_physical=cfg.dt_physical,
).to(device)
