"""V18 Fine-Tune + FNO Decoder training script.

Load V0 pretrained weights → fine-tune with FNO decoder + differential LR.
Encoder/transition from V0, decoder (FNO) is fresh.
"""
...
from versions.v18_finetune_fno.model import FinetuneFNOE2CO
from versions.v18_finetune_fno.loss import FinetuneFNOLoss
...
cfg.version_name = "v18_finetune_fno"
cfg.use_amp = False      # cfloat in SpectralConv2d incompatible with fp16
cfg.use_compile = False  # torch.compile can't handle cfloat params

print(f"=== V18 Fine-Tune + FNO Decoder ===")
...
# Partial checkpoint loading: encoder + transition match V0, decoder (FNO) is fresh
...
loss_fn = FinetuneFNOLoss(cfg).to(device)
