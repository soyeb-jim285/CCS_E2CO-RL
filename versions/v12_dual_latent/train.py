"""V12 Dual Latent Space E2CO training script."""
...
from versions.v12_dual_latent.model import DualLatentE2CO
from versions.v12_dual_latent.loss import DualLatentLoss
...
def add_extra_args(parser):
    ...
    parser.add_argument("--latent_data_dim", type=int, default=12)
    parser.add_argument("--latent_phys_dim", type=int, default=8)
...
cfg.version_name = "v12_dual_latent"
cfg.use_compile = False

print(f"=== V12 Dual Latent Space E2CO ===")
...
print(f"Config: epochs={cfg.epochs}, batch_size={cfg.batch_size}, "
      f"lr={cfg.learning_rate}, nsteps={cfg.nsteps}")
print(f"Latent dims: data={cfg.latent_data_dim}, phys={cfg.latent_phys_dim}, "
      f"total={cfg.latent_data_dim + cfg.latent_phys_dim}")
...
model = DualLatentE2CO(
    latent_data_dim=cfg.latent_data_dim,
    latent_phys_dim=cfg.latent_phys_dim,
    u_dim=cfg.u_dim,
...
# Use total latent dim for loss (it sees z0 = [z_data, z_phys])
loss_fn = DualLatentLoss(
    cfg, perm, device,
