"""V10 Enhanced PINN E2CO training script."""
...
from versions.v10_enhanced_pinn.model import EnhancedPINNE2CO
from versions.v10_enhanced_pinn.loss import EnhancedPINNLoss


def add_extra_args(parser):
    parser.add_argument("--n_collocation_points", type=int, default=1024)
    ...
    parser.add_argument("--siren_omega0", type=float, default=30.0)
    parser.add_argument("--fourier_L", type=int, default=4)
    return {"n_collocation_points": 1024, "lambda_consistency": 0.1,
            "curriculum_start": 60, "curriculum_end": 120,
            "siren_omega0": 30.0, "fourier_L": 4}


cfg = BaseConfig.from_args(extra_args_fn=add_extra_args)
cfg.version_name = "v10_enhanced_pinn"
cfg.use_compile = False
# AMP stays True — autocast disabled only inside PINN decoder

print(f"=== V10 Enhanced PINN E2CO ===")
...
print(f"SIREN: omega0={cfg.siren_omega0}, fourier_L={cfg.fourier_L}")
...
model = EnhancedPINNE2CO(
    ...
    siren_omega0=cfg.siren_omega0,
    fourier_L=cfg.fourier_L,
).to(device)

loss_fn = EnhancedPINNLoss(
    cfg, perm, device,
