"""V10 Enhanced PINN loss — curriculum + pressure-only + bidirectional consistency."""
...
class EnhancedPINNLoss(nn.Module):
    """Curriculum + pressure-only + bidirectional consistency.

    7 loss terms: rec_t0, rec_t1, l2_reg, trans, yobs,
                  pressure_pde_autograd, consistency
    """
    ...
    def __init__(self, cfg, perm_log, device, lambda_consistency=0.1,
        ...
        if self.use_adaptive:
            self.adaptive = AdaptiveLossWeights(num_terms=7)
        else:
            self.adaptive = None
        ...
        lambda_vals = [
            ...
            cfg.lambda_pressure_pde, lambda_consistency,
        ]
    ...
    def forward(self, pred):
        ...
        # Pressure-only autograd physics
        ...
        # Bidirectional consistency: no .detach() on cnn_sampled
        ...
        for i_step in range(len(pinn_preds)):
            ...
            # Bidirectional: both PINN and CNN learn from each other
            loss_consistency = loss_consistency + F.mse_loss(pinn_out, cnn_sampled)
